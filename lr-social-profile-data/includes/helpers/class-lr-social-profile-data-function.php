...
if (!class_exists('LR_Social_Profile_Data_Function')) {
    ...
    class LR_Social_Profile_Data_Function {
        ...
        public static function update_extented_user_profile($userId, $profileData, $access_token = '') {
            global $wpdb, $lr_social_profile_data_settings, $socialLoginObject;
...
//                    print_r($contacts);die;
                    ...
                    $contacts = null;
                    error_log($profileData['Provider'] . ' failed getting (contacts) ' . json_encode($e->getErrorResponse()));
                }
...
//                } catch (\LoginRadiusSDK\LoginRadiusException $e) {
//                    $status = null;
//                    error_log($profileData['Provider'] . ' failed getting (get_status) ' . json_encode($e->getErrorResponse()));
//                }
//
...
//            }
            ...
            if ($profileData['Provider'] == 'twitter' && in_array('mentions', $lr_social_profile_data_settings['profiledata'])) {
                try {
                    ...
                    $mentions = null;
                    error_log($profileData['Provider'] . ' failed getting (get_mentions) ' . json_encode($e->getErrorResponse()));
                }
            ...
            if (in_array($profileData['Provider'], array('facebook', 'vkontakte')) && in_array('groups', $lr_social_profile_data_settings['profiledata'])) {
                try {
                    ...
                    $groups = null;
                    error_log($profileData['Provider'] . ' failed getting (groups) ' . json_encode($e->getErrorResponse()));
                }
            ...
            if ($profileData['Provider'] == 'facebook' && in_array('likes', $lr_social_profile_data_settings['profiledata'])) {
                try {
                    $fblikes = $socialLoginObject->getLikes($access_token);
                } catch (\LoginRadiusSDK\LoginRadiusException $e) {
                    $fblikes = null;
                    error_log($profileData['Provider'] . ' failed getting (likes) ' . json_encode($e->getErrorResponse()));
                }
            ...
            if ($profileData['Provider'] == 'facebook' && in_array('events', $lr_social_profile_data_settings['profiledata'])) {
                try {
                    ...
                    $events = null;
                    error_log($profileData['Provider'] . ' failed getting (events) ' . json_encode($e->getErrorResponse()));
                }
            ...
            if (in_array($profileData['Provider'], array('facebook', 'google', 'live', 'vkontakte')) && in_array('albums', $lr_social_profile_data_settings['profiledata'])) {
                try {
                    $albums = $socialLoginObject->getPhotoAlbums($access_token);
                } catch (\LoginRadiusSDK\LoginRadiusException $e) {
                    $albums = null;
                    error_log($profileData['Provider'] . ' failed getting (albums) ' . json_encode($e->getErrorResponse()));
                }
