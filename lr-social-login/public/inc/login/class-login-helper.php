    ...
    class Login_Helper {
        ...
        public static function verify_user_after_email_confirmation() {
            ...
            if (isset($loginRadiusSettings['LoginRadius_sendemail']) && $loginRadiusSettings['LoginRadius_sendemail'] == 'sendemail') {
                ...
                $message = sprintf(__('Thankyou for registration on our site %s:'), $blogname) . "\r\n\r\n";
                $message .= sprintf(__('Username: %s'), $user->user_login) . "\r\n";
                $message .= sprintf(__('Email: %s'), $user->user_email) . "\r\n";
                $message .= sprintf(__('Password: %s'), $userPassword) . "\r\n";
                ...
                if (!is_wp_error($user_id)) {
                    ...
                    if (!isset($loginradius_api_settings['raas_enable']) || $loginradius_api_settings['raas_enable'] != 1) {
                        ...
                        if (( $sendemail == 'sendemail')) {
                            ...
                            } elseif (isset($loginRadiusSettings['LoginRadius_sendemail']) && $loginRadiusSettings['LoginRadius_sendemail'] == 'sendemail') {
        ...
        public static function get_loginradius_interface_container($isLinkingWidget = false) {
            ...
            if (empty($loginRadiusSecret)) {
                $loginRadiusResult = '';
            } elseif (empty($loginRadiusApiKey) || empty($loginRadiusSecret)) {
        ...
        public static function unlink_provider() {
            ...
            <script type="text/javascript">
                location.href = "<?php echo LR_Common:: get_protocol() . $_SERVER['HTTP_HOST'] . remove_query_arg(array('lrlinked', 'loginradius_linking', 'loginradius_post', 'loginradius_invite', 'loginRadiusMappingProvider', 'loginRadiusMap', 'loginRadiusMain')) ?>?lrlinked=false";
            </script>
            <?php
            die;
        }
        ...
        public static function social_login_interface_beside_registration() {
                ...
                $script .= 'if ( jQuery ( "#loginform" ) .length || jQuery ( "#registerform" ) .length || jQuery ( "#lostpasswordform" ) .length )
			{
				...
				if ( jQuery ( "#lostpasswordform" ) .length ) {
					...
					loginDiv.append ( "<div class=\"login-panel-lr\" style=\"min-height:270px\" >' . self:: display_social_login_interface(true) . '</div>" );
					jQuery ( "#loginform" ) .css ( "height", "270px" );
					jQuery ( "#loginform" ) .css ( "margin-right", "20px" );
				}
			}';
            } elseif ($lrRegister) {
