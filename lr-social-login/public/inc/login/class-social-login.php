...
if ( ! class_exists( 'Social_Login' ) ) {
    ...
    class Social_Login {
        ...
        public function social_login_init() {
            ...
            add_action('login_head', array($this, 'front_end_css'));
        ...
        public static function login_radius_connect() {
            global $wpdb, $loginRadiusSettings, $socialLoginObject, $lr_social_profile_data_settings;
            ...
            if ( ! is_user_logged_in() && isset( $_REQUEST['token'] ) ) {
                ...
                try {
                    ...
                    $responseFromLoginRadius = $socialLoginObject->getUserProfiledata($token); 
                ...
                if ( $userId ) {
                    if(get_user_meta( $userId, self::$loginRadiusProfileData['Provider'] . 'LrVerified', true ) == ""){
                        add_user_meta($userId, self::$loginRadiusProfileData['Provider'] . 'LrVerified', '1');
                    }
                    ...
                    if ( ( 1 == get_user_meta( $userId, self::$loginRadiusProfileData['Provider'] . 'LrVerified', true ) ) || class_exists( 'LR_Raas_Install' ) ) {
                       ...
                       if(!apply_filters('lr_user_allow_login', false)){
        ...
        public function front_end_css() {
            wp_enqueue_style('lr-social-login-style', LR_ROOT_URL . 'lr-social-login/assets/css/lr-social-login-front.css', array());
        }
        ...
        public function replace_default_avatar_with_social_avatar( $avatar, $avuser, $size ,$default, $alt = '' ) {
            ...
            if ( ! empty( $userId ) ) {
                ...
                if( 'socialavatar' == $loginRadiusSettings['LoginRadius_socialavatar'] ) {
                    if ( ( $userAvatar = get_user_meta( $userId, 'loginradius_thumbnail', true ) ) !== false && strlen( trim( $userAvatar ) ) > 0 ) {
                        ...
                        elseif($size == '64'){
                            ...
                            $size = '50';
                            $imgSize = 'style="height:50px;width: 50px"';
                        }
                    }
                ...
                $imgStyle = isset($imgSize)? $imgSize:'';
                return '<img alt="' . esc_attr( $alt ) . '" src="' . $userAvatar . '" class="avatar avatar-' . $size . '" height="' . $size . '" width="' . $size . '" '.$imgStyle.' />';
            }
            return $avatar;
        }
        /**
