...
if (!class_exists('LR_Raas_Social_Login')) {
    ...
    class LR_Raas_Social_Login {
        ...
        public static function login_script($linkingWidget = false, $custom = false) {
            global $loginradius_api_settings, $lr_raas_settings, $lr_custom_interface_settings;
            ...
            if (!empty($loginradius_api_settings['LoginRadius_apikey'])) {
        ...
        public static function get_interface($newInterface = false) {
            global $loginradius_api_settings, $LR_Raas_Social_Login;
            ...
            if (empty($loginRadiusApiKey) || empty($loginRadiusSecret)) {
        ...
        public static function raas_forms($page) {
            ...
            <?php } elseif ($page == 'registration') { ?>
                ...
                if (!empty($lr_raas_settings['email_verify_option']) && 'disabled' == $lr_raas_settings['email_verify_option'] || !empty($lr_raas_settings['email_verify_option']) && 'optional' == $lr_raas_settings['email_verify_option']) {
                    ?>
        ...
        public static function raas_user_updation($errors, $update, $user) {
            ...
            if (false == $update) {
                ...
                try {
                    ...
                    $response = $userAPIObject->create($params);
                ...
                if (!empty($accountId)) {
                    ...
                    try {
                        $response = $userAPIObject->edit($accountId, $params);
                        if (isset($response->Uid) && !empty($response->Uid)) {
                            try {
                                $userProfile = $userAPIObject->getProfileByID($accountId);
                        ...
                        $errors->add('user_updation_error', $e->getErrorResponse()->description);
                    }
                }
            }
        }
        ...
        public static function save_extra_profile_fields($user_id) {
            ...
            if (!empty($accountId)) {
                try {
                    $response = $userAPIObject->edit($accountId, $params);
                    if (isset($response->isPosted) && !empty($response->isPosted)) {
                        try {
                            $userProfile = $userAPIObject->getProfileByID($accountId);
                            if (isset($userProfile->ID)) {
                    ...
                    update_user_meta($user_id, 'lr_profile_update_error', $e->getErrorResponse()->description);
        ...
        public static function display_extra_profile_fields($user) {
            ...
            <table class="form-table">
                <tr>
                    ...
                    <td>
                        <input type="text" name="email" id="email" value="<?php echo esc_attr(get_the_author_meta('email', $user->ID)); ?>" class="regular-text" <?php
            if ($user->ID) {
        ...
        public static function change_password($data) {
            ...
            try {
                ...
                $message = '<div style="color:red;">' . $e->getErrorResponse()->description . '</div>';
            }
        ...
        public static function set_password($data) {
            ...
            $params = array('accountid' => $uid, 'emailid' => $email, 'password' => $password);
            try {
                $accountAPIObject->createUserRegistrationProfile($params);
                $message = '<div style="color:green;">' . __('Password has been Created Successfully.') . '</div>';
            } catch (\LoginRadiusSDK\LoginRadiusException $e) {
                $message = '<div style="color:red;">' . $e->getErrorResponse()->description . '</div>';
            }
            update_user_meta($user_id, 'lr_message_text', $message);
