...
if (!class_exists('LR_Custom_Obj_Install')) {
    ...
    class LR_Custom_Obj_Install {
        ...
        public static function set_default_options($blog_id) {
            ...
            if ($blog_id) {
                if (!get_blog_option($blog_id, 'LR_Raas_Custom_Obj_Settings')) {
                    update_blog_option($blog_id, 'LR_Raas_Custom_Obj_Settings', self::custom_obj_options());
                }
                $lr_raas_custom_obj_settings = get_blog_option($blog_id, 'LR_Raas_Custom_Obj_Settings');
            } else {
