...
if (!class_exists('LR_Custom_Object_Admin_Settings')) {
    ...
    class LR_Custom_Object_Admin_Settings {
        ...
        function load_custom_scripts() {
            ...
            wp_enqueue_script('lr_customobject_admin_script', LR_ROOT_URL . 'lr-custom-object/assets/js/lr_customobject.js', array('jquery'), LR_PLUGIN_VERSION, $lr_js_in_footer);
        }
                        ...
                        <?php foreach ($lr_Custom_Obj_Fields as $field) { ?>
                            <div style="clear: both;">
                                ...
                                <span class="custom_option">
                                    ...
                                    <input type="text" style="float:left;" class="lr-row-field" id="lr-custom-textbox-<?php echo $field; ?>" name="LR_Raas_Settings[custom_<?php echo $field; ?>_title]" placeholder="<?php _e('Custom Object Field ' . $field . ' Name', 'lr-plugin-slug'); ?>" value="<?php echo isset($lr_raas_custom_obj_settings['custom_' . $field . '_title']) ? $lr_raas_custom_obj_settings['custom_' . $field . '_title'] : ''; ?>" />
                            ...
                            <script>
                                jQuery(document).ready(function () {
                                    jQuery('#lr-show-raas-custom-field-<?php echo $field; ?>').click(function () {
                                        if (jQuery(this).prop("checked") == true) {
                                            jQuery('#lr-custom-textbox-<?php echo $field; ?>').prop("required", true);
                                        } else if (jQuery('#lr-show-raas-custom-field-<?php echo $field; ?>').prop("checked") == true && jQuery('#lr-custom-textbox-<?php echo $field; ?>').val() == '') {
                                            jQuery('#lr-custom-textbox-<?php echo $field; ?>').prop("required", true);
                                        } else {
                                            jQuery('#lr-custom-textbox-<?php echo $field; ?>').prop("required", false);
                                        }
                                    });
                                });
                            </script>
