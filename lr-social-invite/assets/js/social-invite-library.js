...
if (chkLrContact != null) {
	...
	$LRSI.resetForm = function() {
		...
		if( si_custom_subject.value ) {
			$('#lr_si_subject').val( si_custom_subject.value );
		} else {
			$('#lr_si_subject').val('');
		}

		if( si_custom_message.value ) {
			$('#lr_si_message').val( si_custom_message.value );
		} else {
			$('#lr_si_message').val('');
		}
		...
		$.ajax( {
			...
			error: function ( xhr, textStatus, errorThrown ) {
				alert( 'Your Token has been expired' );
                                jQuery('.lr_social_invite_results').hide();
			}
		} );
	};
		...
		$.ajax( {
			...
			success: function ( data_contacts, textStatus, XMLHttpRequest ) {
					...
					$.each( contacts, function( index, key ){
						...
						if(key.image_url == '' || key.image_url == null) {
	...
	function SocialInviteSuccess( element ) {
			...
			if( token != null && provider != null ) {
				$.ajax( {
					...
					error: function ( xhr, textStatus, errorThrown ) {
						console.log( 'Unexpected error occurred1' );
					}
				} );
			}
	};
	...
	function loginMessage( event ) {
		...
		if( token.length > 0 && currentprovider.length > 0 ){
			...
			$.ajax( {
				...
				error: function ( xhr, textStatus, errorThrown ) {
					console.log( 'Update Token: Unexpected error occurred2' );
				}
			} );
